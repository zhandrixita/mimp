(function () {
  ...
  // Se lee desde .page (no document.documentElement): .tema-extranjeras
  // sobreescribe --header-green-1/--header-green-2 con el verde agua propio
  ...
  function bars(dict, scrollable) {
    var items = entries(dict).sort(function (a, b) { return b.data.casos - a.data.casos; });
    var max = items.length ? items[0].data.casos : 1;
    var filas = '<div class="extranjeras-bars">' + items.map(function (item) {
      return '<div class="extranjeras-bar"><span>' + esc(item.label) + '</span><div class="extranjeras-bar-track"><div class="extranjeras-bar-fill" style="width:' +
        (item.data.casos / max * 100) + '%"></div></div><strong>' + fmt.format(item.data.casos) + ' (' + item.data.pct + '%)</strong></div>';
    }).join("") + "</div>";
    return scrollable ? '<div class="extranjeras-bars-scroll">' + filas + '</div>' : filas;
  }
  ...
  function buildPictogramSvg(rawSvg, colorVar, uid) {
    ...
    var clipId = "pictoclip-extranjeras-" + uid;
    var svg =
  ...
  function bloqueSexo(etiqueta, sexo, d, idPrefix) {
    var pictoHostId = "extranjeras-edad-pictos-" + idPrefix;
    var indicadores = [
      ["Discapacidad", d.indicadores.discapacidad],
      ["Persona gestante", d.indicadores.gestando],
      ["Persona LGBTI", d.indicadores.lgtbi]
    ];
    return {
      ...
      html:
        ...
        '<div class="extranjeras-grid">' +
        card("Grupo de edad", '<div id="' + pictoHostId + '" class="grid grid-3"></div>') +
        indicadores.map(function (item) { var yes = yesValue(item[1]); return card(item[0], '<div class="extranjeras-stat"><strong>' + yes.pct + '%</strong><span>' + fmt.format(yes.casos) + ' casos con respuesta S&iacute;</span></div>' + bars(item[1])); }).join("") +
        card("Nivel de riesgo", bars(d.nivel_riesgo)) +
        card("V&iacute;nculo con la presunta persona agresora", bars(d.vinculo_agresor)) +
        card("Tipo de violencia", bars(d.tipo_violencia)) +
        card("Pa&iacute;s de origen", bars(d.pais_origen, true), "extranjeras-wide") +
        "</div>",
    };
  }

  // Mapa de calor por quintiles derivado del color de tema (mismo mecanismo
  // que alcohol.js/lgtbi.js/main_v2.js): 5 tonos del acento de la pestana
  // activa, de claro a oscuro -- asi el mapa de "Personas extranjeras" queda
  // en su propio verde agua sin tocar el color de las otras pestanas.
  ...
  function _renderLeyendaQuintiles(quintiles, maxVal, paleta) {
    ...
    var host = document.getElementById("extranjeras-map-legend-rows");
    if (!host) return;
    host.innerHTML = "";
    for (var i = 0; i < 5; i++) {
      var row = document.createElement("div");
  ...
  function renderMap(data) {
    ...
    echarts.registerMap("PERU_EXTRANJERAS", window.GEODATA_DEPT);
    ...
    chart("extranjeras-map-chart").setOption({
      tooltip: {
        trigger: "item",
        formatter: function (p) { return p.name + ": <strong>" + fmt.format(p.value || 0) + "</strong> casos"; }
      },
      series: [{
        type: "map", map: "PERU_EXTRANJERAS", roam: true, nameProperty: "nombdep", label: { show: false },
    ...
    var mapChart = chart("extranjeras-map-chart");
    window.renderMapLabels(mapChart, data.por_departamento, "extranjeras");
    mapChart.on("georoam", function () { window.refreshMapLabels("extranjeras"); });
  }

  function renderRegionBars(data) {
    var host = document.getElementById("extranjeras-region-bars");
    if (!host) return;
    host.innerHTML = "";
  ...
  function wireMapToggle(host) {
    host.querySelectorAll(".extranjeras-map-toggle-btn").forEach(function (btn) {
      btn.addEventListener("click", function () {
        host.querySelectorAll(".extranjeras-map-toggle-btn").forEach(function (b) { b.setAttribute("aria-selected", "false"); });
        btn.setAttribute("aria-selected", "true");
        var vista = btn.getAttribute("data-view");
        var mapView = document.getElementById("extranjeras-map-view");
        var barsView = document.getElementById("extranjeras-region-bars");
        mapView.hidden = vista !== "mapa";
        barsView.hidden = vista !== "barras";
        if (vista === "mapa" && charts["extranjeras-map-chart"]) {
          charts["extranjeras-map-chart"].resize();
          window.refreshMapLabels("extranjeras");
        }
      });
    });
  }

  window.renderExtranjeras = function (data) {
    var host = document.getElementById("extranjeras-dashboard");
    if (!host || !data) return;
    ...
    host.innerHTML = '<p class="extranjeras-intro"><strong>' + fmt.format(data.total) +
      ...
      '<button type="button" class="view-toggle-btn extranjeras-map-toggle-btn" data-view="mapa" aria-selected="true">Mapa</button>' +
      '<button type="button" class="view-toggle-btn extranjeras-map-toggle-btn" data-view="barras" aria-selected="false">Barras</button>' +
      '</div></div>' +
      '<div class="card-window-body">' +
      '<div class="map-host-wrap" id="extranjeras-map-view">' +
      '<div class="map-host" id="extranjeras-map-chart"></div>' +
      '<div class="map-legend" id="extranjeras-map-legend"><div class="map-legend-title">Casos</div><div class="map-legend-rows" id="extranjeras-map-legend-rows"></div></div>' +
      '</div>' +
      '<div class="region-bars" id="extranjeras-region-bars" hidden></div>' +
      '</div></div>' +
      '<div class="theme-panel-top">' +
      card("Hombres y mujeres", '<div class="extranjeras-sexo-resumen">' +
        '<div class="extranjeras-stat"><strong>' + fmt.format(data.hombres.total) + '</strong><span>Hombres &middot; ' + data.hombres.pct_del_total + '%</span></div>' +
        '<div class="extranjeras-stat"><strong>' + fmt.format(data.mujeres.total) + '</strong><span>Mujeres &middot; ' + data.mujeres.pct_del_total + '%</span></div>' +
        '</div>') +
      '</div>' +
      ...
      '<div class="card-window-header"><span>Casos por mes</span><div class="view-toggle" id="extranjeras-historico-toggle"></div></div>' +
      '<div class="card-window-body"><div id="extranjeras-historico-chart" class="extranjeras-chart"></div></div>' +
      '</div>' +
      '<p class="extranjeras-note">Los porcentajes utilizan como denominador los casos filtrados de personas extranjeras (por sexo, sobre el total de esa secci&oacute;n).</p>' +
      '</div></div>';
    ...
    window.renderHistoricoMensual({
      chartId: "extranjeras-historico-chart", toggleHostId: "extranjeras-historico-toggle",
  ...
  window.resizeExtranjerasCharts = function () {
    Object.keys(charts).forEach(function (id) { charts[id].resize(); });
    if (charts["extranjeras-map-chart"]) window.refreshMapLabels("extranjeras");
  };
  window.addEventListener("resize", window.resizeExtranjerasCharts);
})();
